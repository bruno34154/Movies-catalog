...
export default function DetailMovie(){
    const {id} = useParams();
    const [movie, setMovie] = useState<Movie>({title: '', poster_path: '', id: '', vote_average: '', overview: '', release_date: new Date()});
    const api_key: string = (process.env.REACT_APP_API_KEY as string)
    const release = new Date(movie.release_date).toLocaleString('pt-BR').split(',')
    useEffect(()=>{
        axios.get(`https://api.themoviedb.org/3/movie/${id}?api_key=${api_key}&language=pt-BR`).then((res)=>{
        ...
        <div>
            ...
            <div className='flex p-5 sm:flex-col'>
                ...
                <div className='text-white  ml-5  flex flex-col'>
                    ...
                    <div className='text-white  ml-5  mt-5 flex'>Sinopse: {movie.overview}</div>
                    <div className='text-white  ml-5  mt-5 flex'>data de lançamento: {release[0]}</div>
                </div>
